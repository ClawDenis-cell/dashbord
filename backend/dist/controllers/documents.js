...
function markdownToHtml(md) {
    let html = md
        ...
        const dataRows = rowLines.map((line) => line.split('|').map((c) => c.trim()).filter(Boolean));
...
exports.DocumentController = {
    async getAll(req, res) {
        try {
            ...
            const docs = await documents_1.DocumentModel.findAll(req.userId, folderId || undefined);
            res.json(docs);
        }
        catch (error) {
    ...
    async getById(req, res) {
        try {
            if (!req.userId)
                return res.status(401).json({ error: 'Not authenticated.' });
            const { id } = req.params;
            const access = await documents_1.DocumentModel.canAccess(id, req.userId);
            if (!access)
                return res.status(404).json({ error: 'Document not found.' });
            const doc = await documents_1.DocumentModel.findById(id);
            const collaborators = await documents_1.DocumentModel.getCollaborators(id);
    ...
    async getByProject(req, res) {
        try {
            ...
            const docs = await documents_1.DocumentModel.findByProjectId(projectId, req.userId);
            res.json(docs);
        }
        catch (error) {
    ...
    async getRecent(req, res) {
        try {
            ...
            const docs = await documents_1.DocumentModel.getRecentForUser(req.userId, limit);
            res.json(docs);
        }
        catch (error) {
    ...
    async create(req, res) {
        try {
            ...
            const doc = await documents_1.DocumentModel.create({
                title,
                content: content || '',
                owner_id: req.userId,
                project_id,
    ...
    async update(req, res) {
        try {
            ...
            const doc = await documents_1.DocumentModel.update(id, req.body);
            res.json(doc);
        }
        catch (error) {
    ...
    async delete(req, res) {
        try {
            ...
            await documents_1.DocumentModel.delete(id);
    ...
    async addCollaborator(req, res) {
        try {
            ...
            const collab = await documents_1.DocumentModel.addCollaborator(id, user_id, permission);
    ...
    async removeCollaborator(req, res) {
        try {
            ...
            if (access !== 'owner')
                return res.status(403).json({ error: 'Only the owner can manage collaborators.' });
            await documents_1.DocumentModel.removeCollaborator(id, userId);
    ...
    async createInvite(req, res) {
        try {
            ...
            const invite = await documents_1.DocumentModel.createInvite(id, permission || 'read', expires_in_hours || 72);
    ...
    async acceptInvite(req, res) {
        try {
            ...
            const invite = await documents_1.DocumentModel.findInviteByToken(token);
            if (!invite)
                return res.status(404).json({ error: 'Invite not found or expired.' });
            const doc = await documents_1.DocumentModel.findById(invite.document_id);
            if (!doc)
                return res.status(404).json({ error: 'Document not found.' });
            if (doc.owner_id === req.userId) {
                return res.json({ message: 'You are the owner of this document.', document: doc });
            }
            await documents_1.DocumentModel.addCollaborator(invite.document_id, req.userId, invite.permission);
    ...
    async uploadImage(req, res) {
        try {
            ...
            const uploadPath = path_1.default.join(UPLOAD_DIR, 'documents', id);
            if (!fs_1.default.existsSync(uploadPath)) {
                fs_1.default.mkdirSync(uploadPath, { recursive: true });
            }
            fs_1.default.writeFileSync(path_1.default.join(uploadPath, filename), req.file.buffer);
            const image = await documents_1.DocumentModel.saveImage({
                document_id: id,
                filename,
    ...
    async uploadImageUrl(req, res) {
        try {
            ...
            let ext = path_1.default.extname(urlPath) || '.png';
            ...
            fs_1.default.writeFileSync(path_1.default.join(uploadPath, filename), imageBuffer);
            const mimeTypes = {
                '.png': 'image/png',
                '.jpg': 'image/jpeg',
                '.jpeg': 'image/jpeg',
            ...
            const image = await documents_1.DocumentModel.saveImage({
                document_id: id,
                filename,
                original_name: path_1.default.basename(urlPath) || `image${ext}`,
    ...
    async getFolders(req, res) {
        try {
            if (!req.userId)
                return res.status(401).json({ error: 'Not authenticated.' });
            const folders = await documents_1.DocumentModel.getFolders(req.userId);
            res.json(folders);
        }
        catch (error) {
    ...
    async createFolder(req, res) {
        try {
            ...
            const folder = await documents_1.DocumentModel.createFolder(name, req.userId, parent_id || null);
    ...
    async updateFolder(req, res) {
        try {
            if (!req.userId)
                return res.status(401).json({ error: 'Not authenticated.' });
            const { id } = req.params;
            const folder = await documents_1.DocumentModel.getFolderById(id);
            if (!folder || folder.user_id !== req.userId) {
                return res.status(404).json({ error: 'Folder not found.' });
            }
            const updated = await documents_1.DocumentModel.updateFolder(id, req.body);
            res.json(updated);
        }
        catch (error) {
    ...
    async deleteFolder(req, res) {
        try {
            ...
            await documents_1.DocumentModel.deleteFolder(id);
